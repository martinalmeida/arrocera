...
async function cargarTablaAsync(){
    let info;
    const data = await fetch("../../model/trillador/table.php");
    const resulatdo = await data.json();
    info = resulatdo.map(e => {
        return `<tr>
                    <td>${e.id_trillador}</td>
                    <td>${e.nombre}</td>
                    <td>${e.cedula}</td>
                    <td>${e.telefono}</td>
                    <td>${e.whatsapp}</td>
                    <td class="text-center">
                        <button type="button" class="btnEditar btn btn-success" data-bs-toggle="modal" data-bs-target="#update_modal" onclick="updateForm('${e.id_trillador}');">Edit <i class="far fa-edit text-white"></i></button> 
                        <button class="btn btn-danger" onclick="eliminaregistro('${e.id_trillador}');" >Delete <i class="far fa-minus-square text-white"></i></button>
                    </td>
                </tr>`;        
    }).join('');
...
window.addEventListener('load', () => {
    ...
    function data() {
        ...
        fetch('../../model/trillador/insert.php', {
            method: 'POST',
            body: datos
...
function eliminaregistro(id) {
    ...
    fetch("../../model/trillador/delete.php", {
        method: "POST",
        body: id
...
function updateForm(id) {
    fetch("../../model/trillador/select_update.php", {
        method: "POST",
        body: id
    }).then(response => response.json()).then(response => {
        id_form.value = response.id_trillador;
...
window.addEventListener('load', () => {
    ...
    function data() {
        ...
        fetch('../../model/trillador/update.php', {
            method: 'POST',
            body: datos
        }).then(Response => Response.json())
        .then(({ success }) => {
