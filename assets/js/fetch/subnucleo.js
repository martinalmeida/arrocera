...
async function cargarTablaAsync(){
    let info;
    const data = await fetch("../../model/subnucleo/table.php");
    const resulatdo = await data.json();
    info = resulatdo.map(e => {
        return `<tr>
                    <td>${e.id_subnucleo}</td>
                    <td>${e.nombre}</td>
                    <td>${e.referente}</td>
                    <td class="text-center">
                        <button type="button" class="btnEditar btn btn-success" data-bs-toggle="modal" data-bs-target="#update_modal" onclick="updateForm('${e.id_subnucleo}');">Edit <i class="far fa-edit text-white"></i></button> 
                        <button class="btn btn-danger" onclick="eliminaregistro('${e.id_subnucleo}');" >Delete <i class="far fa-minus-square text-white"></i></button>
                    </td>
                </tr>`;        
    }).join('');
...
window.addEventListener('load', () => {
    ...
    let fk_referente_sub = document.getElementById('fk_referente_sub');
    ...
    function data() {
        ...
        datos.append("fk_referente_sub", fk_referente_sub.value);
        fetch('../../model/subnucleo/insert.php', {
            method: 'POST',
            body: datos
...
function eliminaregistro(id) {
    ...
    fetch("../../model/subnucleo/delete.php", {
        method: "POST",
        body: id
...
let fk_referente_sub_up = document.getElementById('fk_referente_sub_up');

function updateForm(id) {
    fetch("../../model/subnucleo/select_update.php", {
        method: "POST",
        body: id
    }).then(response => response.json()).then(response => {
        id_form.value = response.id_subnucleo;
        nombre_up.value = response.nombre;
        fk_referente_sub_up.value = response.fk_referente_sub;
    })
}
...
window.addEventListener('load', () => {
    ...
    function data() {
        ...
        datos.append("fk_referente_sub_up", fk_referente_sub_up.value);
        fetch('../../model/subnucleo/update.php', {
            method: 'POST',
            body: datos
        }).then(Response => Response.json())
        .then(({ success }) => {
