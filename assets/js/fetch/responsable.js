...
async function cargarTablaAsync(){
    let info;
    const data = await fetch("../../model/responsable/table.php");
    const resulatdo = await data.json();
    info = resulatdo.map(e => {
        return `<tr>
                    <td>${e.id_responsable}</td>
                    <td>${e.nombre}</td>
                    <td>${e.cedula}</td>
                    <td>${e.telefono}</td>
                    <td>${e.whatsapp}</td>
                    <td class="text-center">
                        <button type="button" class="btnEditar btn btn-success" data-bs-toggle="modal" data-bs-target="#update_modal" onclick="updateForm('${e.id_responsable}');">Edit <i class="far fa-edit text-white"></i></button> 
                        <button class="btn btn-danger" onclick="eliminaregistro('${e.id_responsable}');" >Delete <i class="far fa-minus-square text-white"></i></button>
                    </td>
                </tr>`;        
    }).join('');
...
window.addEventListener('load', () => {
    ...
    function data() {
        ...
        fetch('../../model/responsable/insert.php', {
            method: 'POST',
            body: datos
...
function eliminaregistro(id) {
    ...
    fetch("../../model/responsable/delete.php", {
        method: "POST",
        body: id
...
function updateForm(id) {
    fetch("../../model/responsable/select_update.php", {
        method: "POST",
        body: id
    }).then(response => response.json()).then(response => {
        id_form.value = response.id_responsable;
...
window.addEventListener('load', () => {
    ...
    function data() {
        ...
        fetch('../../model/responsable/update.php', {
            method: 'POST',
            body: datos
        }).then(Response => Response.json())
        .then(({ success }) => {
