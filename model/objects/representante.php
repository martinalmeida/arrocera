...
class crud
{
    ...
    private $conn;
    private $table_name = "representante";
    ...
    {
        $query = "SELECT * FROM " . $this->table_name . " WHERE id_representante  =  " . $this->data . " ";
    ...
    function create()
    {
        $query = "INSERT INTO " . $this->table_name . " SET nombre_repre=?, cedula=?, telefono=?, correo=?, whatsapp=? ";
        $stmt = $this->conn->prepare($query);
        $this->nombre_repre = htmlspecialchars(strip_tags($this->nombre_repre));
        ...
        $stmt->bindParam(1, $this->nombre_repre);
    ...
    function makeUpdate()
    {
        $query = "UPDATE " . $this->table_name . " SET nombre_repre=?, cedula=?, telefono=?, correo=?, whatsapp=?  WHERE id_representante = " . $this->id . " ";
        $stmt = $this->conn->prepare($query);
        $this->nombre_repre = htmlspecialchars(strip_tags($this->nombre_repre));
        $this->cedula = htmlspecialchars(strip_tags($this->cedula));
        $this->telefono = htmlspecialchars(strip_tags($this->telefono));
    ...
    function delete()
    {
        $query = "DELETE FROM " . $this->table_name . " WHERE id_representante =  " . $this->data . " ";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
